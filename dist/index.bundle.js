(() => {
        ...
        function r(n, r) {
          for (var a = {}, i = [], s = 0; s < n.length; s++) {
            ...
            if (-1 !== u) t[u].references++, t[u].updater(p);
              ...
              var m = o(p, r);
              (r.byIndex = s),
                t.splice(s, 0, { identifier: f, updater: m, references: 1 });
            }
            i.push(f);
          }
          return i;
  ...
  (() => {
    ...
    const g = (() => {
        ...
        const t = document.querySelector("#add-task"),
          e = JSON.parse(localStorage.getItem("Task-list")) || [];
        return (
          t.addEventListener("click", () => {
            const t = document.querySelector("#add-task-input").value;
            if ("" === t) return;
            const r = new n(t);
            e.push(r), localStorage.setItem("Task-list", JSON.stringify(e));
          }),
          e
        );
      })(),
      k = () => g.map((n) => n),
      v = document.querySelector(".task-list-container");
    k().forEach((n) => {
      v.innerHTML += `<li class="container task flex-center" draggable="true">\n    <span class="left flex-center">\n     <input id=${
        n.index
      } type="checkbox" ${
        n.completed ? "checked" : ""
        ...
        n.completed ? "edit-task disabled" : "edit-task"
      }" ${
        n.completed ? "disabled" : ""
      }\n       >\n     </form>\n    </span>\n    <span class="right">\n      <i class="fas fa-ellipsis-v"></i>\n      <i class="far fa-trash-alt"></i>\n    </span>\n   </li>`;
    });
    const y = document.querySelectorAll(".task"),
      b = document.querySelectorAll(".edit-task"),
      x = document.querySelectorAll(".edit-form"),
      w = document.querySelector(".reload"),
      S = document.querySelectorAll(".checkbox");
    x.forEach((n) => {
      n.addEventListener("submit", (n) => {
        n.preventDefault(),
          b.forEach((n) => {
            k().forEach((t) => {
              n.dataset.indexNumber === t.index &&
                ((t.description = n.value),
                localStorage.setItem("Task-list", JSON.stringify(k())));
            });
          });
      });
    }),
      window.addEventListener("load", () => {
        S.forEach((n) => {
          ...
          (() => {
            const n = document.querySelector(".task-list-container");
            document.querySelectorAll(".task").forEach((n) => {
              ...
              n.addEventListener("dragover", (e) => {
                e.preventDefault();
                const r = t(n, e.clientY),
                  o = document.querySelector(".dragging");
                null === r && n.appendChild(o), n.insertBefore(o, r);
              });
            const t = (n, t) =>
              [...n.querySelectorAll(".task:not(.dragging)")].reduce(
                (n, e) => {
                  const r = e.getBoundingClientRect(),
                    o = t - r.top - r.height / 2;
                  return o < 0 && o > n.offset ? { offset: o, element: e } : n;
                },
      ...
      y.forEach((n) => {
        n.addEventListener("click", () => {
          y.forEach((n) => n.classList.remove("focus")),
            n.classList.add("focus");
        });
      }),
      document.querySelector(".clear-task").addEventListener("click", () => {
        if (k().length > 0) {
          const n = k().filter((n) => !0 !== n.completed);
          localStorage.setItem("Task-list", JSON.stringify(n)),
            window.location.reload();
        }
      }),
      y.forEach((n) => {
        n.addEventListener("click", () => {
          ...
          n.classList.contains("focus") &&
            ((t = n.querySelector(".far")),
            (e = n.querySelector(".checkbox").id),
            t.addEventListener("click", () => {
              const n = k().filter((n) => n.index !== e);
              localStorage.setItem("Task-list", JSON.stringify(n)),
                window.location.reload();
            }));
        });
